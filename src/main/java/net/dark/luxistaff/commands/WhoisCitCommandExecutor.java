...
public class WhoisCitCommandExecutor implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length >= 1) {
                ...
                String vulcanCommand = "vulcan profile " + playerName;

                player.performCommand(vulcanCommand);
                return true;
            } else {
                sender.sendMessage(ColorUtil.translate(RGBUtil.translateHexColorCodes(PREFIX + "Utilizzo corretto: /whoiscit <player>")));
                return true;
            }
        } else {
