...
public class CheatLogCommandExecutor implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if (args.length != 1) {
                player.sendMessage(ColorUtil.translate(RGBUtil.translateHexColorCodes(PREFIX + "Utilizzo corretto: /cheatlog <player>")));
                return true;
            }

            String playerName = args[0];
            String vulcanCommand = "vulcan violations " + playerName;

            player.performCommand(vulcanCommand);
            return true;
